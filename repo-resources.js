(async function(){
  ...
  const typeFrom=(parts,name)=>{
    const folders=parts.map(x=>x.toLowerCase()); const n=name.toLowerCase();
  ...
  const schoolFrom=(parts,type)=>{
    if(type!=='Assessments')return '';
    const idx=parts.findIndex(x=>/^(assessments?|tests?)$/i.test(x.trim()));
